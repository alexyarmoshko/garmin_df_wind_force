...
- **User settings** (Milestone 5): Configurable via Garmin Connect Mobile or Garmin Express — wind units (Beaufort, Knots, mph, km/h, m/s), forecast interval 1 (1-6h), forecast interval 2 (1-6h). Changes invalidate cached forecasts and take effect on the next background fetch.
- **Staleness indicator**: Prefixes display with `*` when data is older than 30 minutes (e.g., `*3/4NE<5/6S`).
- **Display engine** (Milestone 3): Adaptive layout showing 1-3 time slots depending on data field width and content length. Auto font sizing selects largest readable font; slot count dynamically reduces if text overflows.
- **Cloudflare Worker proxy** (Milestone 2): Translates Met Eireann XML to compact JSON with KV caching. Supports unit conversion and slot selection server-side. API is versioned under `/v1` prefix; all responses include `api_version` field.
